...
1. **通用激活语固定那一句**（Coze / Meoo / WorkBuddy 三平台）。不为任何平台改文案、不加平台后缀。可移植性 = 一切。
   - ⚠️ **MIAODA 例外**：百度秒哒安全策略拦截「读取外部文件激活技能」类指令（2026-07-21 实测）。MIAODA 改用 `docs/MIAODA-SELF-CONTAINED-BRIEF.md` 自包含内联任务书（已内联 NEXL 身份+六步法+`base.min.html` 源码），不依赖 fetch 外部。这是唯一对"固定激活语"原则的偏离，因平台安全策略不可绕过。
...
| **MIAODA（百度秒哒）** | App Builder Skill + 「秒点」计费概念 + 全栈生成（前端/后端/数据库）+ LGUI 可视化编辑器。**⚠️ 安全策略拦截"读取外部文件激活技能"，须用 `docs/MIAODA-SELF-CONTAINED-BRIEF.md` 自包含任务书替代通用激活语** |
...
| **百度秒哒 MIAODA** | 🔴 通用激活语被安全策略拦截 → 改用 `docs/MIAODA-SELF-CONTAINED-BRIEF.md` 内联任务书（含 `base.min.html` 源码）；全栈原生直接基于该基底改造输出 web 应用 |
