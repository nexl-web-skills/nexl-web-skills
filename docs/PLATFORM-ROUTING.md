---
date: 2026-07-20
relay: 观涛虾🦐 (TideShell) · 第 16 棒 / 三平台路由架构师
topic: nexl-builder 三平台识别与路由机制（实测修正：Coze 不认 fetch 激活，需内联人设；激活语仅 WorkBuddy 有效）
---

> 本文件为第 16 棒。上一棒：第 15 棒（WorkBuddy 白皮书档案）。下一棒预期：把本路由机制落地为各平台实测（在 Coze/Meoo/WorkBuddy 分别发激活语，验证自识别成功率），或做 nexlbase 中台跨平台预算可视化网页。

# nexl-builder 三平台识别与路由机制 · 接力者必读

当主人把同一句固定激活语：
```
请阅读 https://raw.githubusercontent.com/nexl-web-skills/nexl-web-skills/main/builder.md 并激活 nexl-builder 技能。
```
分别发给 **Coze / 秒悟 Meoo / 腾讯 WorkBuddy** 三个平台的 Agent，每个 Agent 都应：① 识别自己身处哪个平台 → ② 调用 GitHub 上对应平台的规则档案 → ③ 校验当前时间、刷新积分/计费制度 → ④ 服务建站。

本文讲清「如何实现」，作为三平台落地的架构根基。

---

## 一、不变式（设计根基，绝不妥协）

1. **激活模型二分（实测结论）**：① **WorkBuddy（本地 agent）** 用铁律激活语 fetch `builder.md` 当技能定义 → 有效；② **Coze（云端 Bot）** 无"激活技能"语义，会把 `builder.md` 当知识总结 → **改用 `docs/COZE-BOT-PERSONA.md` 人设模板内联**（不发激活语）。激活语仅 WorkBuddy 通用，Coze 一律用人设模板，二者零改动。
...
4. **三平台预算/规则零耦合**。各平台独立 `budget-PLATFORM.json` + `budget-PLATFORM.cjs` + `docs/PLATFORM-WHITEPAPER.md`，互不影响。
...
若自省信号 ambiguous（例如某平台 Agent 未暴露环境特征），**直接问用户**：「你正在用哪个平台建站？① Coze ② Meoo ③ WorkBuddy」。绝不阻塞、绝不臆测。
...
> 接力者注：未来可加一个 CI / 云函数定时抓取三平台官网定价，自动更新 `snapshotDate` 与费率并 PR 回仓库，实现「真·自动校验」。当前阶段靠人工快照 + 30 天警示足矣。
...
**激活语零改动（仅 WorkBuddy）**——本地 agent 平台用户永远只记一句；Coze 等云端 Bot 平台改用自包含人设模板（同样零改动）。两类模型并存，平台无限扩展。
...
- [ ] 三平台分别实测激活语，记录自识别成功率（填表回流）。
- [ ] 把「30 天时效警示」做成标准模板，写入各 `budget-*.cjs` 输出。
- [ ] nexlbase 中台做跨平台预算可视化网页（三平台切换 → 实时积分/秒点 + ¥ + 对比条）。
...
被激活的 agent 在三平台内调出「最优秀 / 性价比最高 / 代码最稳定」模板的统一机制。
