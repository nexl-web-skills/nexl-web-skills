...
author: NEXL
handoff: 21
subject: 四平台案例调用索引 + GitHub 开发者共建模板库的规则设计与执行
---
...
本文件定义两件事：
1. **案例调用索引**：被激活的 agent 如何在四平台内调出「最优秀 / 性价比最高 / 代码最稳定」的模板，作为用户建站的参照锚点。
...
**为什么是这份 schema**：`ratings` 三维对应主人要的「最优秀 / 性价比最高 / 代码最稳定」；`cost_estimate` 让 agent 在对应平台直接报预算；`platforms` 让四平台 agent 只调自己能跑的。
...
被激活的 agent 在四平台中如何「调出最优模板」——这是让 NEXL 用户远超普通入口人群的关键杠杆之一。
...
- **MIAODA**：App Builder Skill 内嵌 INDEX；全栈原生直接 clone 模板 scaffold。
...
**未做（待主人确认）**：四平台白皮书各自加「案例调用」小节（Coze/Meoo/WorkBuddy/MIAODA 各一段）；更多种子模板（目标 5 个覆盖五类目）。
